...
function App() {
  ...
  const userNameError = useMemo(() => {
    if (submitted && !/^[a-zA-Z0-9-]{4,}$/.test(values.userName)) {
      return "At least 4 characters, letters/numbers/dashes only";
    }
    return "";
  ...
  const passwordError = useMemo(() => {
    if (submitted && values.password.length < 8) {
  ...
  const confirmPasswordError = useMemo(() => {
    if (submitted && values.password !== values.confirmPassword) {
